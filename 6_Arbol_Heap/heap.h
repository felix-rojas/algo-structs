/*
 * heap.h
 *
 *  Created on: 20/10/2015
 *      Author: clase
 */

#ifndef HEAP_H_
#define HEAP_H_

#include <iostream>
#include <iterator>
#include <sstream>
#include <string>

template <class T> class Heap {
private:
  T *data;
  unsigned int heap_size;
  unsigned int count;

  unsigned int parent(unsigned int) const;
  unsigned int left(unsigned int) const;
  unsigned int right(unsigned int) const;
  void heapify(unsigned int);
  void swap(unsigned int, unsigned int);

public:
  Heap(unsigned int);
  ~Heap();
  bool empty() const;
  bool full() const;
  void push(T);
  unsigned int size();
  void pop();
  T top();
  void clear();
  std::string toString() const;
};

template <class T> Heap<T>::Heap(unsigned int sze) {
  data = new T[sze];
  if (data == 0) {
    throw "OutOfMemory";
  }
  heap_size = sze + 1;
  count = 1;
}

template <class T> Heap<T>::~Heap() {
  delete[] data;
  data = 0;
  heap_size = 0;
  count = 1;
}

template <class T> bool Heap<T>::empty() const { return count == 1; }
template <class T> unsigned int Heap<T>::size() { return count-1; }

template <class T> bool Heap<T>::full() const { return count == heap_size; }

template <class T> unsigned int Heap<T>::parent(unsigned int pos) const {
  return pos / 2;
}

template <class T> unsigned int Heap<T>::left(unsigned int pos) const {
  return pos * 2;
}

template <class T> unsigned int Heap<T>::right(unsigned int pos) const {
  return (pos * 2) + 1;
}

template <class T> void Heap<T>::swap(unsigned int i, unsigned int j) {
  T aux = data[i];
  data[i] = data[j];
  data[j] = aux;
}

template <class T> void Heap<T>::heapify(unsigned int pos) {
  unsigned int le = left(pos);
  unsigned int ri = right(pos);
  unsigned int min = pos;
  if (le <= count && data[le] < data[min]) {
    min = le;
  }
  if (ri <= count && data[ri] < data[min]) {
    min = ri;
  }
  if (min != pos) {
    swap(pos, min);
    heapify(min);
  }
}

template <class T> void Heap<T>::push(T val) {
  unsigned int pos;
  if (full()) {
    throw "NoSuchElement";
  }
  pos = count;
  count++;
  while (pos > 0 && val < data[parent(count)]) {
    data[pos] = data[parent(pos)];
    pos = parent(pos);
  }
  if (pos == 0) {
    data[1] = val;
  } else {
    data[pos] = val;
  }
}

template <class T> T Heap<T>::top() {
  if (empty()) {
    throw "NoSuchElement";
  }
  T aux = data[1];
  return aux;
}

template <class T> void Heap<T>::pop() {
  if (empty()) {
    throw "NoSuchElement";
  }
  data[1] = data[--count];
  heapify(1);
}

template <class T> void Heap<T>::clear() { count = 1; }

template <class T> std::string Heap<T>::toString() const {
  std::stringstream aux;

  aux << "[";
  for (unsigned int i = 1; i < count; i++) {
    if (i != 1) {
      aux << " ";
    }
    if (data[i] != 1)
      aux << data[i];
  }
  aux << "]";
  return aux.str();
}
#endif /* HASH_H_ */
