...
#define ASSERT_FALSE(condition)                                                \
  assertFalse(condition, __FILE__, __LINE__, #condition);
...
void UnitTest::assertTrue(bool condition, const char *file, int line,
                          const char *msg) {
  if (!condition) {
    std::cout << " FAILURE.\n";
    std::cout << file << ":" << line << ": " << msg
              << ", number of tests successful: " << num_test_sucess
              << " grade = " << ((num_test_sucess / (double)total_test) * 100)
              << " of 100\n";
    exit(1);
  }
  ++num_test_sucess;
...
void UnitTest::assertFalse(bool condition, const char *file, int line,
                           const char *msg) {
  if (condition) {
    std::cout << " FAILURE.\n";
    std::cout << file << ":" << line << ": " << msg
